...
test("URLパラメータ系の関数は互換性を持つ", 1, function(){
  ...
  var url = "http://example.com/?" + app.url.build_param(original_data);
  deepEqual(app.url.parse_query(url), original_data);
});

module("app.url.fix", {
  setup: function(){
    this.fixed_board_url = [
...
test("URLを解析して、その情報を返す", 15, function(){
...
module("app.url.tsld", {
  setup: function(){
    this.test = function(url, expected){
